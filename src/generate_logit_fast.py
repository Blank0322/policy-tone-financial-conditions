...
def stage_d_logit_index():
    ...
    # Load only necessary columns with Polars
    df = pl.read_parquet(daily_path).select(["month", "text_clean"]).to_pandas()
    ...
    # Limit text length for faster vectorization
    df['text_short'] = df['text_clean'].str.slice(0, 10000)
    
    df['exp_hits'] = df['text_short'].str.count(exp_regex)
    df['tig_hits'] = df['text_short'].str.count(tig_regex)
    ...
    # 2. Train Logistic Regression (Simplified)
    vectorizer = TfidfVectorizer(max_features=2000, analyzer='char')
    X = vectorizer.fit_transform(train_df['text_short'])
    y = train_df['label']
    ...
    X_all = vectorizer.transform(df['text_short'])
